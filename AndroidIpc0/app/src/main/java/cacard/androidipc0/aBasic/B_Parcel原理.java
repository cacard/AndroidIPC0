package cacard.androidipc0.aBasic;

/**
 * Created by cunqingli on 2017/2/24.
 */
 ...
 * 一个可以通过IBinder发送数据的容器。
 * 可携带flattened数据，在IPC另一侧unflattend。
 *
 ...
 * Parcel的目的并不是作为一种序列化机制。设计的目的是为了IPC高性能数据传输。
 * 所以，不应该把Parcel数据进行持久化存储：数据结构发生的变化，那么旧数据就不可读了。
 *
